<?php
/**
 * Template part for displaying new works (Maddahi)
 */

$args = array(
    'post_type' => 'heyat_media',
    'posts_per_page' => get_theme_mod('count_media_section', 10),
    'tax_query' => array(
        array(
...
?>
<!-- 4. NEW WORKS & CONTENT CAROUSEL -->
<section id="mediaSection" class="reveal-on-scroll my-10 relative">
    <div class="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-goldAccent/10 text-goldAccent flex items-center justify-center text-xl border border-goldAccent/20">
                <?php heyat_render_section_icon('media_section', 'fa-compact-disc'); ?>
            </div>
            <div class="flex flex-col text-right">
                <h3 class="text-base md:text-lg font-bold text-white tracking-wide"><?php echo esc_html(get_theme_mod('title_media_section', 'جدیدترین آثار منتشر شده')); ?></h3>
                <span class="hidden md:block text-[10px] text-textMuted font-medium mt-0.5"><?php echo esc_html(get_theme_mod('subtitle_media_section', 'آرشیو جدیدترین صوت‌ها و ویدیوها')); ?></span>
            </div>
        </div>
    ...
    <div id="maddahiCarousel" class="relative overflow-hidden py-6">
        <div class="carousel-track flex gap-4 transition-transform duration-500 ease-in-out">
            ...
            while ($media_query->have_posts()) : $media_query->the_post(); 
                ...
                // Get duration from repeater if possible
                $files = get_post_meta(get_the_ID(), '_heyat_media_files', true);
                $duration = '--:--';
                $media_url = '';
                if (!empty($files) && is_array($files)) {
                    $duration = !empty($files[0]['duration']) ? $files[0]['duration'] : $duration;
                    $media_url = !empty($files[0]['url']) ? $files[0]['url'] : '';
                }
                
                // Get assigned maddah (maybe from a meta or just author. we'll use excerpt or author name)
            ...
            <div onclick="window.location.href='<?php the_permalink(); ?>'"
                ...
                <div class="flex justify-between items-center mt-2.5 pt-2.5 border-t border-white/5 text-xs text-textMuted">
                    ...
                    <span class="text-goldAccent font-bold"><i class="fa-solid <?php echo esc_attr($play_icon); ?>"></i> <?php echo esc_html($format_text); ?></span>
                </div>
            </div>
            <?php endwhile; ?>
        </div>
